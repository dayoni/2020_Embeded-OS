...
#define INIT_MSG	"=======================================\nHello! Da-Yeon & Sang-Chu Project #3...\nPlease, LOG-IN!\n---------------------------------------\n"
...
void *clnt_login(void *arg) {
	...
	// 연결된 소켓으로부터 아이피, 포트 받아오는부분
	rcv_byte = recv(new_fd, ip, sizeof(ip), 0);
	rcv_byte = recv(new_fd, port, sizeof(port), 0);
	...
	if ((strcmp(id, USER1_ID) == 0) && (strcmp(pw, USER1_PW) == 0)) {
		printf("%s\n", OK_LOG1);
		// 배열 초기화
		user_ip[0][0] = '\0'; user_port[0][0] = '\0';
		send(new_fd, OK_LOG1, strlen(OK_LOG1) + 1, 0);
		users_num = 1;

		// 전역변수에서 기다리고 있는 아이피, 포트 변수를 각각의 유저에 맞게 저장
		sprintf(user_ip[0], "%s", ip); sprintf(user_port[0], "%s", port);
		// 다른 유저의 아이피, 포트가 들어왔을 경우 혹시나를 대비한 배열 초기화
		ip[0] = '\0';
		port[0] = '\0';
	}
	// USER2 정상 로그인
		...
		printf("%s\n", OK_LOG2);
		//배열 초기화
		user_ip[1][0] = '\0'; user_port[1][0] = '\0';
		send(new_fd, OK_LOG2, strlen(OK_LOG2) + 1, 0);
		users_num = 2;

		// 전역변수에서 기다리고 있는 아이피, 포트 변수를 각각의 유저에 맞게 저장
		sprintf(user_ip[1], "%s", ip); sprintf(user_port[1], "%s", port);
		// 다른 유저의 아이피, 포트가 들어왔을 경우 혹시나를 대비한 배열 초기화
		ip[0] = '\0';
		port[0] = '\0';
	}
	// USER3 정상 로그인
		...
		printf("%s\n", OK_LOG3);
		// 배열 초기화
		user_ip[2][0] = '\0'; user_port[2][0] = '\0';
		send(new_fd, OK_LOG3, strlen(OK_LOG3) + 1, 0);
		users_num = 3;

		// 전역변수에서 기다리고 있는 아이피, 포트 변수를 각각의 유저에 맞게 저장
		sprintf(user_ip[2], "%s", ip); sprintf(user_port[2], "%s", port);
		// 다른 유저의 아이피, 포트가 들어왔을 경우 혹시나를 대비한 배열 초기화
		ip[0] = '\0';
		port[0] = '\0';
	}
...
void send_msg(char *msg, int num) {
	...
	// [FILE]이라는 메시지 받았을 경우 클라이언트-클라이언트 에게 클라이언트-서버의 아이피, 포트 전송하는 부분
	if (strstr(msg, "[FILE]")) {
		// 자신보다 늦게 생성된 클라이언트 소켓으로 우선 [FILE] 이라는 메시지 전송
		for (int i = 0; i < num; i++) {
			send(clnt_socks[i], msg, strlen(msg) + 1, 0);
		}
		// 자신보다 먼저 생성된 클라이언트 소켓으로 우선 [FILE] 이라는 메시지 전송
		...
		// 전송할 아이피, 포트 버퍼에 저장
		char serv_ip_buf[100];
		char serv_port_buf[100];
		sprintf(serv_ip_buf, "%s", user_ip[num]);
		sprintf(serv_port_buf, "%s", user_port[num]);

		// 자신보다 늦게 생성된 클라이언트 소켓으로 자신의 아이피, 포트 전송
		for (int i = 0; i < num; i++) {
			send(clnt_socks[i], serv_ip_buf, strlen(serv_ip_buf) + 1, 0);
			send(clnt_socks[i], serv_port_buf, strlen(serv_port_buf) + 1, 0);
		}
		// 자신보다 먼저 생성된 클라이언트 소켓으로 자신의 아이피, 포트 전송
		for (int j = clnt_cnt; j > num; j--) {
			send(clnt_socks[j], serv_ip_buf, strlen(serv_ip_buf) + 1, 0);
			send(clnt_socks[j], serv_port_buf, strlen(serv_port_buf) + 1, 0);
		}
	}
	else {
